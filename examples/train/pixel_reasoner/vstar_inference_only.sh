#!/bin/bash
# Pure Inference Script for VStar - No Training Components
# Optimized for 2x RTX 4090

set -x

echo "=========================================="
echo "VStar Pure Inference - 2x RTX 4090"
echo "=========================================="
echo ""
...
if [ -d "/mnt/data/pixel_reasoner_3b" ]; then
    ...
    echo "✓ Step 1/6: Using model at $model_name"
else
    model_name=VerlTool/pixel-reaoner-3b-grpo-n8-b128-t1.0-lr1e-6-complex-reward_global_step_100
    echo "✓ Step 1/6: Using HuggingFace model"
fi

# Dataset configuration
if [ -f "/mnt/data/pixel_reasoner/vstar/test.parquet" ]; then
    dataset_path="/mnt/data/pixel_reasoner/vstar/test.parquet"
    echo "✓ Step 2/6: Using dataset at $dataset_path"
else
    echo "✗ Error: Dataset not found at /mnt/data/pixel_reasoner/vstar/test.parquet"
    exit 1
fi

# Start tool server
echo "✓ Step 3/6: Starting tool server..."
...
server_pid=$!
sleep 5
echo "  Tool server running at $tool_server_url (pid=$server_pid)"

# Run pure inference
echo "✓ Step 4/6: Loading model and initializing VLLM..."
echo "✓ Step 5/6: Starting inference on VStar dataset..."
...
PYTHONUNBUFFERED=1 python3 -m verl_tool.trainer.main_ppo \
    ...
    trainer.project_name=vstar_inference \
    trainer.experiment_name=vstar_pure_inference \
